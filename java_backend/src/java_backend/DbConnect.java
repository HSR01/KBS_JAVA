...
public class DbConnect {
    ...
    public Object[][] getPersonen(int ID) {
        try {
            ...
            //haal alles op.
            Object[][] returnval = new Object[aantal][7];
            query = "SELECT P.PersoonID, P.Voornaam, P.Tussenvoegsel, P.Achternaam, Tr.TrajectID, Tr.Begin, Tr.Eind "
                    + "FROM Persoon P "
                    + "JOIN Traject_BPS T ON P.PersoonID = T.PersoonID "
                    + "JOIN Traject Tr ON T.TrajectID = Tr.TrajectID";
            rs = st.executeQuery(query);
            int i = 0;
            while (rs.next()) {
                returnval[i][0] = rs.getString("PersoonID");
